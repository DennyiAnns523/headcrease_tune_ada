...
class CallBackVerification(object):
    """
    Evaluation callback: runs verification on a protocol file (.bin) at a specified frequency.
    """
    def __init__(
        self,
        frequent: int,
    ...
    def init_dataset(
        ...
        for name in val_targets:
            ...
            if os.path.exists(path):
                # load_bin takes (path, image_size) to resize & flip exactly as in repo
        ...
        with torch.no_grad(), autocast():
            for idx, data_set in enumerate(self.ver_list):
                acc1, std1, acc2, std2, xnorm, _ = verification.test(
                    data_set, norm_model, batch_size=10, nfolds=10
                )
                name = self.ver_name_list[idx]
                logging.info(f"[{name}][{global_step}] Accuracy-Flip: {acc2:.5f}±{std2:.5f}")
                if acc2 > self.highest_acc_list[idx]:
                    self.highest_acc_list[idx] = acc2
                logging.info(f"[{name}][{global_step}] Accuracy-Highest: {self.highest_acc_list[idx]:.5f}")
                results.append(acc2)
...
class CallBackLogging(object):
    ...
    """
    def __init__(self, frequent: int, rank: int, total_step: int, batch_size: int, world_size: int, writer=None):
        self.frequent = frequent
        self.rank = rank
    ...
    def __call__(self, global_step: int, loss_meter, epoch: int):
        ...
        speed = (self.frequent * self.batch_size * self.world_size / elapsed) if elapsed > 0 else float('inf')
        ...
        if hasattr(loss_meter, 'avg'):
            ...
            logging.warning("CallBackLogging: could not extract numeric from loss_meter, defaulting to 0.0")
            loss_value = 0.0
...
class CallBackModelCheckpoint(object):
    """
    Checkpoint callback: saves model & header only when the supplied metric improves.
    """
    def __init__(self, rank: int, output: str = "./"):
        self.rank = rank
        self.output = output
    ...
    def __call__(
        ...
        if metric > self.best_metric:
            ...
            backbone_path = os.path.join(self.output, f"best_backbone_epoch{epoch:02d}.pth")
            torch.save(backbone.module.state_dict(), backbone_path)
            logging.info(f"[Metric {old_best:.5f} → {metric:.5f}] Saved new best backbone: {backbone_path}")

            if header is not None:
                header_path = os.path.join(self.output, f"best_header_epoch{epoch:02d}.pth")
                torch.save(header.module.state_dict(), header_path)
                logging.info(f"[Metric {old_best:.5f} → {metric:.5f}] Saved new best header:   {header_path}")

...
from torch.utils.checkpoint import checkpoint  # <-- Add this import at the top
...
class IResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        def layer2_forward(x):
            with torch.cuda.amp.autocast():
                return self.layer2(x)
        
        def layer3_forward(x):
            with torch.cuda.amp.autocast():
                return self.layer3(x)
        
        def layer4_forward(x):
            with torch.cuda.amp.autocast():
                return self.layer4(x)  

        x = torch.utils.checkpoint.checkpoint(layer1_forward, x)
        x = torch.utils.checkpoint.checkpoint(layer2_forward, x)
        x = torch.utils.checkpoint.checkpoint(layer3_forward, x)
        x = torch.utils.checkpoint.checkpoint(layer4_forward, x)      
